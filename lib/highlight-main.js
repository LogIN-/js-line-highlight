 ...
 * @Last Modified time: 2014-10-26 10:55:07
 */
        ...
        if (match.length === 4) {
            ...
            if (match[1] === self.elementID) {
                ...
                if (match[2] && match[3]) {
                    ...
                    var desiredElement = parseInt(match[2]);
                    var desiredLine = parseInt(match[3]);
                    ...
                    // 2. Claculate element rendered STYLE and totalRows
                    self.initElementDimensions();
...
lineHighlight.prototype.highlightRow = function() {
    ...
    if (self.bookmarkEvent === true) {
        // 1. Get coordinates
        var position = self.getElementOffset(self.element.el);
        // 2. Scroll to desired element
        self.scrollTo(document.documentElement, position.x, 75);

 ...
 * @Last Modified time: 2014-10-25 10:58:44
 */
...
var lineHighlight = function(element) {
    ...
    self.options = {
        ...
        ignoredEl: ['UL', 'A', 'IMG', 'CANVAS', 'INPUT', 'SELECT', 'TEXTAREA', 'STRONG', 'EM'],
    ...
    // Current active DOM element properties
    self.element = {
        ...
        if (match.length === 4) {
            ...
            if (match[1] === self.elementID) {
                ...
                if (match[2] && match[3]) {
                    ...
                    var desiredElement = match[2];
                    var desiredLine = match[3];



                    // 2. Set element as default
                    self.element.el = self.childElements[desiredElement];

                    // 3. Get coordinates
                    // self.getPosition();

                    // 1. Scroll to desired element
                    self.scrollTo(document.documentElement, 0, 75)

                    // 4. Call highlighter

                    console.log("dispatchEvent");
                    // self.FireEvent(self.childElements[desiredElement], 'mouseenter');
    ...
    for (var i = 0; i < self.childElements.length; i++) {
        ...
        self.childElements[i].addEventListener('mousechange', function(e) {
            ...
            if (currentPos !== self.currentIndex || (currentPos === 1 && self.currentIndex === 1)) {
                ...
                console.log(self.element.totalRows);
...
lineHighlight.prototype.getPosition = function() {
    ...
    this.element.X = borderLeftWidth - rect.left;
    this.element.Y = borderTopWidth - rect.top;
};
...
lineHighlight.prototype.getLineUrl = function() {
    return window.location.protocol + '//' + window.location.host + window.location.pathname + "#I" + this.elementID + "S" + this.element.childNumber + "L" + this.currentIndex;
};
